...
# koordinator-runbookens coordinator-runbook.md §7 (merge + opprydding) (gate a) + coordinator-runbook.md §1 (kø-utvelgelse) (--next).
...
#   (3) Premature-archive — PARKED ∩ arkiv (coordinator-runbook.md §7 (arkivering)-kontraktbrudd — arkivert før
...
#
# Kjøres fra repo-roten (slik CI-jobben og koordinatorens coordinator-runbook.md §7 (merge + opprydding)-kjøring gjør det)
...
if [ "$mode" = "--next" ]; then
  ...
  ref=${TODO_NR_REF:-origin/{{BASE_BRANCH}}}
...
if [ -s "$premature_file" ]; then
  echo "WARN: premature-archive (coordinator-runbook.md §7 (arkivering)-kontraktbrudd) — PARKED-nr har allerede en arkiv-gravstein, gater IKKE:" >&2
  while IFS= read -r nr; do
    [ -z "$nr" ] && continue
