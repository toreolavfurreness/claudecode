...
# fra koordinator-runbookens coordinator-runbook.md §7 (merge + opprydding) (gate b), som SISTE handling før `gh pr merge`.
#
...
# koordinatorer (se coordinator-runbook.md §7 (merge + opprydding)/coordinator-runbook.md §7 i coordinator-runbook.md for avveiningen). Kjør denne
...
# runbookens coordinator-runbook.md §7 (merge + opprydding)-punkt a) dekker koordinatorens EGNE ucommitterte coordinator-runbook.md §7 (arkivering)/coordinator-runbook.md §7/§7-
...
# --base    default ${TODO_NR_REF:-origin/{{BASE_BRANCH}}} — samme env-navn som checkeren.
...
#
# Exit-kontrakt (se også runbokens coordinator-runbook.md §7 (merge + opprydding)):
...
#      ALDRI som grønt
#   3  merge-konflikt (ikke en nr-kollisjon i seg selv) → coordinator-runbook.md §7 (merge + opprydding)-pausen gjelder,
#      sjekken ble ikke kjørt. UNNTAK (coordinator-runbook.md §7 (merge + opprydding)/renummererings-oppskriften): en
...
base=${base:-${TODO_NR_REF:-origin/{{BASE_BRANCH}}}}
...
if [ "$mt_rc" -eq 1 ]; then
  ...
  printf 'Se coordinator-runbook.md §7 (merge + opprydding) i coordinator-runbook: konfliktstier under tasks/todos/ med SAMME nr+slug\n' >&2
