...
| Superpowers `/simplify` | Post-impl forenkling/DRY-analyse | Tilgjengelig | Ikke tilgjengelig | Kalles eksplisitt i `todo-finish-worker.md` steg 3 (workeren har `Skill` i toolsettet). Mangler skillen → inline DRY-gjennomgang av diffen i samme steg |
| Superpowers `/requesting-code-review` | Post-impl kode-review | Tilgjengelig | Ikke tilgjengelig | Utføres allerede inline i `todo-finish-worker.md` steg 5 — ingen ytterligere degradering |
