Les CLAUDE.md, tasks/lessons/index.md, deretter relevante lessons-filer,
tasks/bugs.md og tasks/todo.md i denne rekkefølgen.

Kjør git-orientering:
  git branch --show-current && git log --oneline -5 && git status

Hvis du er på main: opprett en passende feature-branch før du fortsetter.
Hvis du finner ucommittede endringer fra tidligere sesjon: rapporter dette.

Finn første todo som ikke er merket [x] eller [~].
Sjekk om det finnes åpne bugs i tasks/bugs.md som er relevante.

Gi meg en kort statusoppdatering med:
- Nåværende branch og git-status
- Eventuelle funn fra avbrutt sesjon
- Plan for neste todo

Vent på bekreftelse før du begynner å kode.
